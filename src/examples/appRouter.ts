...
export const appRouter = t.router({
  ...
  makeHat: t.procedure
    .use(packageMiddleware({ packages: ['twitch.twirp.example'], options: { go_package: '/example' } }))
  ...
  send: t.procedure
    .use(packageMiddleware({ packages: ['twirp.internal.twirp', 'test.multiple'], options: { go_package: '/multiple' } }))
    .input(Msg1Schema)
    .output(Msg1Schema)
    .mutation(() => {
      return {};
  ...
  execute: t.procedure
    .use(packageMiddleware({ packages: ['network.protocol.storage'], options: { cc_generic_services: 'true' } }))
    .input(z.object({
      ...
      operations: z.array(z.object({
        ...
        rops: z.array(z.object({
          op: z.enum([
            'ROW_DELETE', 'ROW_FETCH', 'COL_DELETE', 'COL_FETCH', 'COL_WRITE', 'COL_MERGE',
            'FLD_CLEAR', 'FLD_FETCH', 'FLD_WRITE', 'FLD_MERGE', 'FLD_INCR', 'FLD_TEST', 'FLD_SCAN',
            'ROW_TEST', 'COL_TEST', 'FLD_SMAX', 'COL_COND', 'FLD_COND', 'COND_POP', 'LOG_DEBUG'
          ]),
          ...
          condition: z.enum([
            'COND_ALWAYS', 'COND_NOT_EXISTS', 'COND_NOT_EQUAL', 'COND_LESS_THAN', 'COND_NOT_GREATER',
            'COND_EQUALS', 'COND_NOT_LESS', 'COND_GREATER_THAN', 'COND_NEVER'
          ]).optional(),
