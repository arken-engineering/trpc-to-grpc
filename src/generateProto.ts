...
export const zodToProtoType = (zodType: ZodTypeAny): string => {
  if (zodType instanceof ZodString) {
    ...
    const fields = Object.entries(zodType.shape).map(([key, value]: any, index) => {
...
export const generateProtoFile = (router: any, outputPath: string) => {
  ...
  let services: Record<string, { definitions: string, packageNames: string[], options: Record<string, string> }> = {};
  ...
  for (const [name, procedure] of Object.entries(procedures)) {
    ...
    protoMessages[inputMessageName] = `message ${inputMessageName} ${zodToProtoType(inputSchema)}\n`;
    protoMessages[outputMessageName] = `message ${outputMessageName} ${zodToProtoType(outputSchema)}\n`;
